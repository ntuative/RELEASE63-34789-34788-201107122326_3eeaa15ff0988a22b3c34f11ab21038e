package com.sulake.habbo.session.furniture
{
   ...
   public class FurnitureDataParser extends EventDispatcher
   {
      ...
      public static const const_139:String = "ready";
      ...
      private var var_1564:Map;
      
      private var _localization:ICoreLocalizationManager;
      
      private var var_786:AssetLibrary;
      ...
      public function FurnitureDataParser(param1:Map, param2:Map, param3:Map, param4:ICoreLocalizationManager)
      {
         ...
         this.var_786 = new AssetLibrary("FurniDataParserAssetLib");
      }
      ...
      {
         var _loc2_:AssetLoaderStruct = this.var_786.loadAssetFromFile(param1,new URLRequest(param1),"text/plain");
         _loc2_.addEventListener(AssetLoaderEvent.ASSET_LOADER_EVENT_COMPLETE,this.parseFurnitureData);
         _loc2_.addEventListener(AssetLoaderEvent.const_41,this.furnitureDataError);
      }
      ...
      private function parseFurnitureData(param1:AssetLoaderEvent) : void
      {
         ...
         for each(_loc10_ in _loc8_)
         {
            ...
            for each(_loc12_ in _loc11_)
            {
               ...
               if(_loc14_ == "s")
               {
                  this.var_533.add(_loc15_,_loc31_);
               }
               else if(_loc14_ == "i")
               {
                  this._wallItems.add(_loc15_,_loc31_);
               }
               _loc32_ = this.var_1564[_loc17_];
               if(_loc32_ == null)
               {
                  _loc32_ = [];
                  this.var_1564.add(_loc17_,_loc32_);
               }
               _loc32_[_loc18_] = _loc15_;
            }
            _loc9_++;
         }
         dispatchEvent(new Event(const_139));
      }
      ...
      public function registerFurnitureLocalizations() : void
      {
         ...
         for each(_loc1_ in this.var_533.getValues())
         {
