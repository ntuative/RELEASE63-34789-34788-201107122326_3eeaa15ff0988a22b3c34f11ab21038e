package com.sulake.habbo.ui.widget.furniture.dimmer
{
   ...
   public class DimmerViewColorGrid
   {
      ...
      private var var_568:IItemGridWindow;
      ...
      public function setSelectedColorIndex(param1:int) : void
      {
         ...
         if(param1 < 0 || param1 >= this.var_568.numGridItems)
         {
            return;
         }
         this.select(this.var_568.getGridItemAt(param1) as IWindowContainer);
      }
      
      private function populate(param1:IHabboWindowManager) : void
      {
         if(this._view == null || this.var_568 == null)
         {
            return;
         }
      ...
      private function populateColourGrid(param1:IHabboWindowManager) : void
      {
         ...
         var _loc13_:* = null;
         var _loc14_:* = null;
         this.var_568.destroyGridItems();
         this.var_237 = null;
         for each(_loc2_ in this.colors)
         {
            _loc3_ = param1.buildFromXML(this.var_2043) as IWindowContainer;
            ...
            _loc3_.width = this.var_708.width;
            _loc3_.height = this.var_708.height;
            this.var_568.addGridItem(_loc3_);
            _loc4_ = _loc3_.findChildByTag("BG_BORDER") as IBitmapWrapperWindow;
            if(_loc4_ != null)
            {
               _loc4_.bitmap = new BitmapData(this.var_708.width,this.var_708.height,true,0);
               _loc4_.bitmap.copyPixels(this.var_708,this.var_708.rect,new Point(0,0));
            }
            _loc5_ = _loc3_.findChildByTag("COLOR_IMAGE") as IBitmapWrapperWindow;
            if(_loc5_ != null)
            {
               _loc5_.bitmap = new BitmapData(this.var_1331.width,this.var_1331.height,true,0);
            ...
            if(_loc6_ != null)
            {
               _loc6_.bitmap = new BitmapData(this.var_1066.width,this.var_1066.height,true,16777215);
               _loc6_.bitmap.copyPixels(this.var_1066,this.var_1066.rect,new Point(0,0),null,null,true);
      ...
      {
         var _loc2_:int = this.var_568.getGridItemIndex(param1.target as IWindow);
