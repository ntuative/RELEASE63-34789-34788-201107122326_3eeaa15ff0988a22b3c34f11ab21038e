package com.sulake.habbo.ui.widget.avatarinfo
{
   ...
   public class AvatarMenuView extends AvatarInfoView
   {
      ...
      protected static const const_1033:uint = 1;
      
      protected static const const_1034:uint = 2;
      
      protected static var var_1434:uint = const_1033;
      
      private static const const_1490:uint = 16744755;
      
      private static const const_1491:uint = 16756591;
      
      private static const const_1072:int = 8;
       
      
      protected var _data:AvatarInfoData;
      
      protected var var_717:uint = 1;
      ...
      protected function updateButtons() : void
      {
         ...
         if(!this.var_1707)
         {
            ...
            {
               this.var_717 = const_1034;
            }
            else
            {
               this.var_717 = var_1434;
            }
         }
         if(this.var_717 == const_1033)
         {
      ...
      private function showButton(param1:String, param2:Boolean = true) : void
      {
         ...
         if(_loc6_)
         {
            ...
            {
               _loc6_.x = _loc5_.x + (_loc5_.width - _loc5_.textWidth) / 2 - _loc6_.width - const_1072;
            }
            if(_loc6_.tags.indexOf("arrow_right") != -1)
            {
               _loc6_.x = _loc5_.x + (_loc5_.width + _loc5_.textWidth) / 2 + const_1072;
            }
         }
      }
      ...
      private function buttonEventProc(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         else if(param1.type == WindowMouseEvent.WINDOW_EVENT_MOUSE_OVER)
         {
            if(param2.name == "button")
            {
               param2.color = param2.tags.indexOf("moderate") > -1 ? uint(const_2048) : uint(const_1035);
            }
            else if(param2.tags.indexOf("link") > -1)
            {
               if(param2.tags.indexOf("actions") > -1)
               {
                  ITextWindow(IWindowContainer(param2).getChildAt(0)).textColor = const_1415;
               }
               else if(param2.tags.indexOf("moderate") > -1)
               {
                  ITextWindow(IWindowContainer(param2).getChildAt(0)).textColor = const_1491;
               }
            }
         }
         else if(param1.type == WindowMouseEvent.const_25)
         {
            if(param2.name == "button")
            {
               param2.color = const_1417;
            }
            else if(param2.tags.indexOf("link") > -1)
            {
               if(param2.tags.indexOf("actions") > -1)
               {
                  ITextWindow(IWindowContainer(param2).getChildAt(0)).textColor = const_1416;
               }
               else if(param2.tags.indexOf("moderate") > -1)
               {
                  ITextWindow(IWindowContainer(param2).getChildAt(0)).textColor = const_1490;
               }
            }
         }
         if(_loc3_)
