package com.sulake.habbo.window
{
   ...
   public class HabboWindowManagerComponent extends Component implements IHabboWindowManager, ICoreWindowManager, IWindowFactory, IUpdateReceiver, IWindowContextStateListener
   {
      ...
      private static const const_470:uint = 3;
      
      private static const const_1425:uint = 1;
      ...
      private var var_1439:IEventDispatcher;
      
      private var var_1157:Boolean = false;
      
      private var var_1727:ProfilerOutput;
      
      private var var_1438:ProfilerAgentTask;
      
      private var var_1437:ProfilerAgentTask;
      ...
      private function initialize(param1:IID, param2:IUnknown) : void
      {
         ...
         this.var_212 = new Array(const_470);
         ...
         var _loc6_:int = 0;
         while(_loc6_ < const_470)
         {
            this.var_212[_loc6_] = new WindowContext("layer_" + _loc6_,this._windowRenderer,this,this._localization,context.displayObjectContainer,_loc5_,this);
            _loc6_++;
         }
         this.var_675 = this.var_212[const_1425];
         unlock();
      ...
      private function receiveProfilerInterface(param1:IID, param2:IUnknown) : void
      {
         ...
         if(_loc3_ != null)
         {
            if(!this.var_1727)
            {
               this.var_1727 = new ProfilerOutput(context,this,this._windowRenderer);
            }
            this.var_1727.profiler = _loc3_;
            this.var_1438 = new ProfilerAgentTask("Update","Event processing");
            _loc3_.getProfilerAgentForReceiver(this).addSubTask(this.var_1438);
            this.var_1437 = new ProfilerAgentTask("Redraw","Window rasterizer");
            _loc3_.getProfilerAgentForReceiver(this).addSubTask(this.var_1437);
            this.var_1157 = true;
         }
      }
      ...
      override public function dispose() : void
      {
         if(!disposed)
         {
            ...
            if(this.var_212)
            {
               while(this.var_212.length > 0)
               {
                  IDisposable(this.var_212.pop()).dispose();
               }
            }
            this.var_212 = null;
            if(this.var_270)
      ...
      {
         var _loc3_:Dictionary = this.var_270[param2] as Dictionary;
         if(_loc3_ == null)
         {
            return null;
         }
         var _loc4_:XML = _loc3_[param1] as XML;
         return _loc4_ == null ? this.var_270[0][param1] as XML : _loc4_;
      }
      ...
      {
         return this.var_212[param9].create(param1,param2,param3,param4,param5,param6,param7,null,param8,null,null);
      }
      
      public function removeWindow(param1:String, param2:uint = 1) : void
      {
         var _loc3_:IDesktopWindow = this.var_212[param2].getDesktopWindow();
      ...
      {
         return this.var_212[param2].getDesktopWindow().getChildByName(param1);
      }
      
      public function getActiveWindow(param1:uint = 1) : IWindow
      {
         return this.var_212[param1].getDesktopWindow().getChildAt(this.var_675.getDesktopWindow().numChildren - 1);
      }
      
      public function getWindowContext(param1:uint) : IWindowContext
      {
         return this.var_212[param1];
      }
      
      public function getDesktop(param1:uint) : IDesktopWindow
      {
         var _loc2_:IWindowContext = this.var_212[param1];
      ...
      public function update(param1:uint) : void
      {
         ...
         if(WindowContext.var_427.length > 0)
         {
            this.var_1439.dispatchEvent(new Event(HabboWindowTrackingEvent.HABBO_WINDOW_TRACKING_EVENT_INPUT));
            _loc2_ = const_470 - 1;
            while(_loc2_ >= 0)
            {
               this.var_212[_loc2_].update(param1);
               _loc2_--;
            }
         }
         if(this.var_1157)
         ...
         this.var_1439.dispatchEvent(new Event(HabboWindowTrackingEvent.HABBO_WINDOW_TRACKING_EVENT_RENDER));
         _loc2_ = 0;
         while(_loc2_ < const_470)
         {
            this.var_212[_loc2_].render(param1);
            _loc2_++;
         }
         if(this.var_1157)
         {
         ...
         if(WindowContext.var_427.length > 0)
         {
            WindowContext.var_427.flush();
         }
         MouseCursorControl.change();
         this.var_1439.dispatchEvent(new Event(HabboWindowTrackingEvent.HABBO_WINDOW_TRACKING_EVENT_SLEEP));
      }
      ...
      public function mouseEventReceived(param1:String, param2:IWindow) : void
      {
         if(param2 != null)
         {
            ...
            {
               ErrorReportStorage.setParameter(HabboErrorVariableEnum.const_1991,new Date().getTime().toString());
               ErrorReportStorage.setParameter(HabboErrorVariableEnum.const_1882,param2.name + ": " + param2.toString());
            }
            else if(param1 == MouseEvent.MOUSE_UP)
            {
               ErrorReportStorage.setParameter(HabboErrorVariableEnum.const_1820,new Date().getTime().toString());
               ErrorReportStorage.setParameter(HabboErrorVariableEnum.const_1974,param2.name + ": " + param2.toString());
            }
         }
      }
