package com.sulake.habbo.communication.demo
{
   ...
   public class HabboCommunicationDemo extends Component
   {
      ...
      private var var_526:IKeyExchange;
      
      private var var_965:String;
      
      private var _view:HabboLoginDemoView;
      
      private var var_2384:PseudoRandom;
      ...
      private var var_241:HabboHotelView;
      
      private var var_1462:String = "";
      
      private const const_2069:String = "fuselogin";
      ...
      public function setSSOTicket(param1:String) : void
      {
         ...
         {
            this.var_625 = param1;
            this.var_62.initConnection(HabboConnectionType.const_268);
         }
      }
      ...
      private function onRoomSessionManagerReady(param1:IID, param2:IUnknown) : void
      {
         ...
         this.var_15.events.addEventListener(RoomSessionEvent.const_82,this.onRoomSessionEnded);
         this.checkRequirements();
      }
      ...
      {
         if(this.var_62 && this.var_151 && this.var_15 && this._windowManager)
         {
      ...
      private function componentsAreRunning() : void
      {
         ...
         var _loc1_:Boolean = this.var_151.getBoolean("client.hotel_view.show_on_startup",true);
         this.var_241 = new HabboHotelView(this._windowManager,assets,Component(context).events,_loc1_);
         var _loc2_:String = this.var_151.getKey("client.hotel_view.image");
         var _loc3_:String = this.var_151.getKey("image.library.url");
         if(_loc2_ != null && _loc3_ != null)
         {
            this.var_241.loadHotelViewImage(_loc3_ + _loc2_);
         }
         var _loc4_:* = this.var_151.getKey("use.sso.ticket") == "1";
         this.var_625 = this.var_151.getKey("sso.ticket",null);
         this.var_1772 = this.var_151.getKey("flash.client.url","");
         this.var_2382 = this.var_151.getKey("external.variables.txt","");
         if(this.var_241 == null)
         {
            this.var_241 = new HabboHotelView(this._windowManager,assets,Component(context).events,_loc1_);
         }
         ...
         else if(this._windowManager != null)
         {
            ...
            if(_loc5_)
            {
               ...
               this._view.addEventListener(HabboLoginDemoView.const_891,this.onInitConnection);
            }
            else
            {
               Core.crash("Login without an SSO ticket is not supported",Core.const_1848);
            }
         }
      }
      ...
      private function onInitConnection(param1:Event = null) : void
      {
         ...
         {
            this.var_62.mode = HabboConnectionType.const_95;
            if(this.var_151.keyExists("local.environment"))
            {
               this.var_62.mode = HabboConnectionType.const_1375;
            }
         }
         this.var_62.initConnection(HabboConnectionType.const_268);
      }
      ...
      private function onConnectionEstablished(param1:Event = null) : void
      {
         ...
         if(_loc2_ != null)
         {
            this.dispatchLoginStepEvent(HabboCommunicationEvent.const_871);
      ...
      private function onInitCrypto(param1:IMessageEvent) : void
      {
         ...
         this.var_2384 = new PseudoRandom(parseInt(_loc4_.substring(_loc4_.length - 4),16),65536);
         switch(this.var_62.mode)
         {
            case HabboConnectionType.const_95:
               _loc6_ = "";
               _loc6_ = this.var_151.getKey("url.prefix",_loc6_);
               _loc6_ = _loc6_.replace("http://","");
               _loc6_ = _loc6_.replace("https://","");
               _loc7_ = this.var_151.getKey("hotelview.banner.url","http:/\nsitename$/gamedata/banner");
               _loc7_ = _loc7_.replace("$sitename$",_loc6_);
               this.var_1462 = _loc4_;
               this.var_241.loadBannerImage(_loc7_ + "?token=" + this.var_1462,this.onHotelViewBannerLoaded);
               break;
            case HabboConnectionType.const_1375:
               this.sendConnectionParameters(_loc2_);
               break;
            case HabboConnectionType.const_1976:
               this.var_965 = this.generateRandomHexString(30);
               this.var_526 = new DiffieHellman(new BigInteger(this.var_151.getKey("connection.development.prime"),16),new BigInteger(this.var_151.getKey("connection.development.generator"),16));
               this.var_526.init(this.var_965);
               _loc8_ = this.var_526.getPublicKey(10);
      ...
      private function onSecretKeyEvent(param1:IMessageEvent) : void
      {
         ...
         this.var_526.generateSharedKey(_loc4_,10);
         var _loc5_:String = this.var_526.getSharedKey(16).toUpperCase();
         var _loc6_:RC4_R27 = new RC4_R27(null,null);
         var _loc7_:IEncryption = new RC4_R27(_loc6_,this.var_2384);
      ...
      private function sendConnectionParameters(param1:IConnection) : void
      {
         ...
         this.var_1461 = true;
         this.dispatchLoginStepEvent(HabboCommunicationEvent.const_296);
         connection.send(new VersionCheckMessageComposer(401,this.var_1772,this.var_2382));
         var machineId:String = "";
         try
         {
            so = SharedObject.getLocal(this.const_2069,"/");
      ...
      private function onSessionParamsEvent(param1:IMessageEvent) : void
      {
         ...
         if(this.var_625 != null)
         {
            _loc4_ = new SSOTicketMessageComposer(this.var_625);
            _loc2_.send(_loc4_);
      ...
      private function onConnectionDisconnected(param1:Event) : void
      {
         ...
         if(param1.type == Event.CLOSE && !this.var_2383)
         {
            _loc2_ = this.var_151.getKey("logout.disconnect.url");
      ...
      private function onDisconnectReason(param1:DisconnectReasonEvent) : void
      {
         ...
         this.var_2383 = true;
         var _loc2_:String = this.var_151.getKey("logout.url");
         if(_loc2_.length > 0)
         {
      ...
      private function setOriginProperty(param1:String) : String
      {
         ...
         {
            return param1.replace("%origin%",this.var_151.getKey("flash.client.origin","popup"));
         }
         return param1;
      }
      ...
      private function onHotelViewBannerLoaded(param1:BitmapData) : void
      {
         ...
         var _loc4_:String = xor(_loc3_,this.var_1462);
         ...
         this.var_526 = new DiffieHellman(_loc10_,_loc11_);
         var _loc13_:int = 10;
         var _loc14_:* = null;
         while(_loc13_ > 0)
         {
            _loc14_ = this.generateRandomHexString(30);
            this.var_526.init(_loc14_);
            _loc15_ = this.var_526.getPublicKey(10);
            if(_loc15_.length >= 64)
            {
               _loc12_ = _loc15_;
               this.var_965 = _loc14_;
            }
