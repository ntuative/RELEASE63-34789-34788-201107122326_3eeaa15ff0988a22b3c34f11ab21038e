package com.sulake.habbo.quest
{
   ...
   public class QuestCompleted implements IDisposable
   {
      ...
      private static const const_1592:int = 31;
      ...
      public function prepare(param1:QuestMessageData) : void
      {
         ...
         this._window.findChildByName("catalog_link_txt").caption = this.var_49.localization.getKey("quests.completed.cataloglink." + this.var_143.activityPointType);
         var _loc2_:String = "quests.completed.reward." + this.var_143.activityPointType;
         this.var_49.localization.registerParameter(_loc2_,"amount",this.var_143.rewardCurrencyAmount.toString());
         this._window.findChildByName("reward_txt").caption = this.var_49.localization.getKey(_loc2_,_loc2_);
         this._window.visible = false;
         this._window.findChildByName("congrats_txt").caption = this.var_49.localization.getKey(!!this.var_143.lastQuestInCampaign ? "quests.completed.campaign.caption" : "quests.completed.quest.caption");
         this._window.findChildByName("more_quests_button").visible = this.var_143.lastQuestInCampaign;
         this._window.findChildByName("campaign_reward_icon").visible = this.var_143.lastQuestInCampaign;
         this._window.findChildByName("catalog_link_region").visible = !this.var_143.lastQuestInCampaign;
         this._window.findChildByName("next_quest_button").visible = !this.var_143.lastQuestInCampaign;
         this._window.findChildByName("reward_icon").visible = !this.var_143.lastQuestInCampaign;
         this._window.findChildByName("campaign_reward_icon").visible = this.var_143.lastQuestInCampaign;
         this._window.findChildByName("campaign_pic_bitmap").visible = this.var_143.lastQuestInCampaign;
         this.setWindowTitle(!!this.var_143.lastQuestInCampaign ? "quests.completed.campaign.title" : "quests.completed.quest.title");
         this.var_49.setupCampaignImage(this._window,param1,this.var_143.lastQuestInCampaign);
         var _loc3_:ITextWindow = ITextWindow(this._window.findChildByName("desc_txt"));
         var _loc4_:int = _loc3_.height;
         this.setDesc(this.var_143.getQuestLocalizationKey() + ".completed");
         _loc3_.height = Math.max(const_1592,_loc3_.textHeight + const_1593);
      ...
      {
         this.var_49.localization.registerParameter(param1,"category",this.var_49.getCampaignName(this.var_143));
      ...
      private function onCatalogLink(param1:WindowEvent, param2:IWindow = null) : void
      {
         if(param1.type == WindowMouseEvent.WINDOW_EVENT_MOUSE_CLICK)
         {
            this.var_49.openCatalog(this.var_143);
         }
      }
      ...
      public function update(param1:uint) : void
      {
         if(this.var_1315 > 0)
         {
            ...
            if(this.var_1315 < 1)
            {
               ...
               if(this.var_143.lastQuestInCampaign)
               {
