package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class NewPetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IGetImageListener
   {
      ...
      private static const const_1464:int = 18;
      ...
      override public function init() : Boolean
      {
         ...
         this.var_536 = this.getPetTypeIndexFromProduct(_loc2_.localizationId);
         if(this.var_536 < 8)
         {
            return false;
         }
         this.var_2467 = _loc2_.localizationId;
         this.updateAvailableBreeds(_loc2_.localizationId);
         if(this.var_228 != null && this.var_228.length > 0)
         {
            this.var_376 = this.var_228[0];
         }
         else
         {
            this.var_376 = -1;
         }
         this._offers.add(this.var_536,_loc2_);
      ...
      private function initializeBreedSelection() : void
      {
         ...
         if(this.var_228 != null)
         {
            _loc1_ = [];
            _loc2_ = 0;
            while(_loc2_ < const_1464 && _loc2_ < this.var_228.length)
            {
               _loc3_ = (page.viewer.catalog as HabboCatalog).roomEngine.getPetColor(this.var_536,this.var_228[_loc2_]);
               if(_loc3_ != null)
               {
      ...
      private function selectedBreed(param1:int) : void
      {
         if(this.var_228 != null && this.var_228.length > 0)
         {
            if(param1 < 0 || param1 > this.var_228.length)
            {
               param1 = 0;
            }
            this.var_376 = this.var_228[param1];
            this.updateImage();
         }
      }
      ...
      private function getPetLocalization() : String
      {
         ...
         if(page != null && false)
         {
            ...
            {
               return _loc1_.localization.getKey(this.getRaceLocalizationKey(this.var_536,this.var_376),this.getRaceLocalizationKey(this.var_536,this.var_376));
            }
         }
         return this.getRaceLocalizationKey(this.var_536,this.var_376);
      }
      ...
      private function updateImage() : void
      {
         ...
         if(_loc1_ == null || this.var_376 < 0)
         {
            return;
         }
         var _loc2_:int = this.var_536;
         var _loc3_:int = this.var_376;
      ...
      private function getPurchaseParameters() : String
      {
         ...
         var p:String = this._name + String.fromCharCode(10) + this.var_376 + String.fromCharCode(10) + this.addZeroPadding(color.toString(16).toUpperCase(),6);
         return p;
      }
