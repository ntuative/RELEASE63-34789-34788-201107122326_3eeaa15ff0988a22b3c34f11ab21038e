package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private var var_1169:Dictionary;
      ...
      private var var_953:int = 3;
      
      private var var_437:AssetLoaderStruct;
      
      private var var_952:Timer;
      ...
      private const const_2342:int = 100;
      
      private const const_1733:int = 2;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         if(_loc6_ == null)
         {
            this.var_437 = this._assets.loadAssetFromFile("figuremap",_loc5_,"text/xml");
         ...
         this.var_952 = new Timer(this.const_2342,1);
         this.var_952.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_437)
         {
            this.var_437.addEventListener(AssetLoaderEvent.ASSET_LOADER_EVENT_COMPLETE,this.onConfigurationComplete);
            this.var_437.addEventListener(AssetLoaderEvent.const_41,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_437)
         {
            this.var_437.removeEventListener(AssetLoaderEvent.ASSET_LOADER_EVENT_COMPLETE,this.onConfigurationComplete);
            this.var_437.removeEventListener(AssetLoaderEvent.const_41,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         if(this.var_953 <= 0)
         {
            HabboWebTools.logEventLog("Figuremap download error " + this.var_1168);
            Logger.log("AVATAR ASSET ERROR: Could not download figuremap.xml.");
         }
         else
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_953);
            if(this.var_1168.indexOf("?") > 0)
            {
               _loc2_ = this.var_1168 + "&retry=" + this.var_953;
            }
            else
            {
               _loc2_ = this.var_1168 + "?retry=" + this.var_953;
            ...
            this.var_437 = this._assets.loadAssetFromFile("figuremap",_loc3_,"text/xml");
            this.addMapLoaderEventListeners();
         }
      }
      ...
      private function loadFigureMapData(param1:XML) : void
      {
         ...
         this.var_1739 = true;
         for each(_loc2_ in this.var_1170)
         {
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2297,this._assets);
            _loc3_.addEventListener(Event.COMPLETE,this.libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = this.var_1169[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(_loc3_);
               this.var_1169[_loc5_] = _loc6_;
            }
         }
      }
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_1739)
         {
            this.var_1170.push([param1,param2]);
            return;
         }
         ...
         if(_loc4_.length > 0)
         {
            ...
            {
               _loc6_ = this.var_582[_loc3_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(param2);
               this.var_582[_loc3_] = _loc6_;
            }
            this.var_951[_loc3_] = _loc4_;
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         var _loc10_:* = null;
         var _loc2_:* = [];
         for(_loc3_ in this.var_951)
         {
            _loc7_ = true;
            _loc4_ = this.var_951[_loc3_];
            ...
            if(_loc7_)
            {
               ...
               delete this.var_582[_loc3_];
            }
         }
         for each(_loc3_ in _loc2_)
         {
            delete this.var_951[_loc3_];
         }
         while(_loc6_ < this.var_768.length)
         {
            _loc5_ = this.var_768[_loc6_];
            if(_loc5_.libraryName == (param1.target as AvatarAssetDownloadLibrary).libraryName)
            {
               this.var_768.splice(_loc6_,1);
            }
            _loc6_++;
         }
         this.var_952.start();
      ...
      {
         var _loc1_:* = null;
         while(this.var_767.length > 0 && this.var_768.length < this.const_1733)
         {
            _loc1_ = this.var_767[0];
            _loc1_.startDownloading();
            this.var_768.push(this.var_767.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_767.indexOf(param1) == -1 && this.var_768.indexOf(param1) == -1)
         {
            this.var_767.push(param1);
            this.processPending();
         }
      }
