package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_1474:IWindow;
      
      private var var_785:IFigurePartSet;
      ...
      private var var_1803:BitmapData;
      
      private const const_2349:int = 13421772;
      
      private const const_2350:int = 16777215;
      
      private const const_389:Array = [2,6,0,4,3,1];
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         this.var_785 = param3;
         this._window = param1;
         this.var_1474 = this._window.findChildByTag("BG_COLOR");
         this._colors = param4;
         this.var_1804 = param5;
         if(param3 == null)
         {
            this.var_747 = new BitmapData(1,1,true,16777215);
         }
         if(param3 != null)
         {
            _loc7_ = param3.parts;
            ...
            {
               this.var_1805 = Math.max(this.var_1805,_loc6_.colorLayerIndex);
            }
         }
         this.var_443 = this.var_78.controller.manager.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_747 != null && !this.var_1804)
            {
               _loc3_ = this.var_747;
            }
            else
         ...
         if(_loc2_)
         {
            if(this.var_785)
            {
               switch(this.var_785.clubLevel)
               {
                  case HabboClubLevelEnum.const_51:
                     _loc2_.visible = false;
                     break;
                  case HabboClubLevelEnum.const_35:
                     _loc2_.style = HabboIconType.const_1015;
                     break;
                  case HabboClubLevelEnum.const_43:
                     _loc2_.style = HabboIconType.const_965;
               }
            }
            else
            {
         ...
         if(this.isSelected)
         {
            this.var_1474.color = this.const_2349;
         }
         else
         {
            this.var_1474.color = this.const_2350;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:IAvatarFigureContainer = this.var_443.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         Logger.log("QUERYING SET:" + this.partSet.type + "-" + this.partSet.id);
         if(!this.var_443.isFigureReady(_loc1_))
         {
            this.var_443.downloadFigure(_loc1_,this);
            return false;
         }
         var _loc6_:int = 0;
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_389[_loc6_] + "_" + FigureData.const_158;
               _loc4_ = this.var_443.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
               while(!_loc7_ && _loc6_ < this.const_389.length)
               {
                  _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_389[_loc6_] + "_" + FigureData.const_158;
                  _loc4_ = this.var_443.getAssetByName(_loc3_) as BitmapDataAsset;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_637 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_1803)
               {
                  ...
                  this.var_1803 = _loc9_.content as BitmapData;
               }
         ...
         _loc1_ = new BitmapData(this.var_637.width,this.var_637.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_637.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_637.y;
