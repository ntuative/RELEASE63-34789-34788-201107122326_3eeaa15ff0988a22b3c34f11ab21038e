package com.sulake.habbo.toolbar
{
   ...
   public class ExtensionView implements IExtensionView
   {
      ...
      private var var_136:IItemListWindow;
      ...
      public function ExtensionView(param1:IHabboWindowManager, param2:IAssetLibrary, param3:HabboToolbar)
      {
         ...
         if(_loc4_)
         {
            this.var_136 = param1.buildFromXML(_loc4_.content as XML,1) as IItemListWindow;
         }
         var _loc5_:IWindowContainer = this.var_136.desktop;
         if(this.var_136)
         {
            this.var_136.x = _loc5_.width - this.var_136.width - const_1062;
            this.var_136.y = const_1062;
            this.var_136.visible = true;
         }
         else
         {
            Core.error("Unable to initialize Toolbar Extension view window from xml asset",false,Core.const_1407);
         }
         this._items = new Map();
      }
      ...
      public function get screenHeight() : uint
      {
         ...
         return this.var_136.height + this.var_136.y;
      }
      ...
      public function attachExtension(param1:String, param2:IWindow, param3:int = -1) : void
      {
         ...
         if(this.var_136)
         {
            if(param3 == -1)
            {
               this.var_136.addListItem(param2);
            }
            else
            {
               this.var_136.addListItemAt(param2,param3);
            }
            this.var_136.invalidate();
         }
      ...
      public function detachExtension(param1:String) : void
      {
         ...
         if(this.var_136)
         {
            this.var_136.removeListItem(_loc2_);
         }
      ...
      private function onResizeTimer(param1:TimerEvent) : void
      {
         if(this._toolbar)
         {
            this._toolbar.events.dispatchEvent(new ExtensionViewEvent(ExtensionViewEvent.const_1281));
         }
      }
   }
}
