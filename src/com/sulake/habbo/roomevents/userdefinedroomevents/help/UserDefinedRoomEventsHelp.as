package com.sulake.habbo.roomevents.userdefinedroomevents.help
{
   ...
   public class UserDefinedRoomEventsHelp
   {
      ...
      private function prepareWindow() : void
      {
         ...
         this._window = IFrameWindow(this.var_93.getXmlWindow("ude_help"));
         this.var_1928 = IWindowContainer(this.find(this._window,"help_container"));
         this.var_1273 = this.var_1928.findChildByName("button_previous") as IContainerButtonWindow;
         if(this.var_1273)
         {
            this.var_1273.procedure = this.onButtonClick;
         }
         this.var_1274 = this.var_1928.findChildByName("button_next") as IContainerButtonWindow;
         if(this.var_1274)
         {
            this.var_1274.procedure = this.onButtonClick;
         }
      ...
      {
         if(this.var_290 >= this.var_75.length - 1)
         {
            this.var_1274.disable();
         }
         else
         {
            this.var_1274.enable();
         }
         if(this.var_290 <= 0)
         {
      ...
      {
         ++this.var_290;
         this.var_290 = Math.min(this.var_290,this.var_75.length - 1);
         this.openPage();
      }
      ...
      {
         --this.var_290;
         this.var_290 = Math.max(this.var_290,0);
         this.openPage();
      }
      ...
      private function initLocalizations() : void
      {
         ...
         this.var_75 = [];
         var _loc1_:int = 1;
         while(this.var_93.localization.getLocalization("wiredfurni.help." + _loc1_ + ".title"))
         {
            _loc2_ = this.var_93.localization.getLocalization("wiredfurni.help." + _loc1_ + ".title");
            _loc3_ = this.var_93.localization.getLocalization("wiredfurni.help." + _loc1_ + ".img");
            _loc4_ = this.var_93.localization.getLocalization("wiredfurni.help." + _loc1_ + ".text");
            this.var_75.push([_loc2_,_loc3_,_loc4_]);
            _loc1_++;
         }
      }
      ...
      private function localize() : void
      {
         ...
         if(!this.var_75 || this.var_75.length == 0)
         {
            return;
         }
         if(_loc1_)
         {
            _loc1_.text = ILocalization(this.var_75[this.var_290][0]).value;
         }
         if(_loc2_)
         {
            _loc2_.text = ILocalization(this.var_75[this.var_290][2]).value;
         }
         if(_loc3_)
         {
            _loc4_ = ILocalization(this.var_75[this.var_290][1]).value;
      ...
      {
         if(!param1 || !this.var_93 || !this.var_93.assets)
         {
            return;
         }
         var _loc2_:BitmapDataAsset = this.var_93.assets.getAssetByName(param1) as BitmapDataAsset;
         if(_loc2_ == null)
         {
      ...
      private function retrievePreviewAsset(param1:String) : void
      {
         ...
         var _loc2_:String = this.var_93.configuration.getKey("image.library.catalogue.url");
         var _loc3_:* = _loc2_ + param1 + ".gif";
         Logger.log("[ProductViewCatalogWidget] Retrieve Product Preview Asset: " + _loc3_);
         var _loc4_:URLRequest = new URLRequest(_loc3_);
         var _loc5_:AssetLoaderStruct = this.var_93.assets.loadAssetFromFile(param1,_loc4_,"image/gif");
         if(!_loc5_)
         {
            return;
         }
