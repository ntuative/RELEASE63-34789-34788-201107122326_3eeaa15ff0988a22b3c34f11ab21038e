package com.sulake.core.utils
{
   ...
   public class LibraryLoaderQueue extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1733:int = 4;
      ...
      {
         return this.var_493.length + this.var_313.length;
      }
      ...
      private function next() : void
      {
         ...
         if(!disposed)
         {
            while(this.var_313.length < const_1733 && this.var_493.length > 0)
            {
               _loc1_ = this.var_493.shift();
               this.var_313.push(_loc1_);
               _loc1_.resume();
            }
         }
      }
      ...
      private function removeLoader(param1:LibraryLoader) : void
      {
         ...
         try
         {
            index = this.var_493.indexOf(loader);
            if(index > -1)
            {
               this.var_493.splice(index,1);
            }
            index = this.var_313.indexOf(loader);
            if(index > -1)
            {
               this.var_313.splice(index,1);
            }
         }
         catch(e:Error)
         {
      ...
      private function isUrlInQueue(param1:String, param2:Boolean = true) : Boolean
      {
         ...
         if(!disposed)
         {
            ...
            for each(_loc3_ in this.var_493)
            {
               if(param2)
               {
      ...
      public function findLibraryLoaderByURL(param1:String, param2:Boolean = true) : LibraryLoader
      {
         ...
         if(!disposed)
         {
            ...
            for each(_loc3_ in this.var_313)
            {
               if(param2)
               {
                  if(_loc3_.url.indexOf(param1) == 0)
