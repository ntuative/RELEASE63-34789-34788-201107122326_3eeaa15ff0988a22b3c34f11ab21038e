package com.sulake.core.utils
{
   ...
   public class LibraryLoader extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_2040:uint = 1;
      
      protected static const const_2041:uint = 2;
      
      protected static const const_2043:uint = 4;
      
      protected static const const_2042:uint = 3;
      ...
      public static const const_1180:Boolean = false;
      
      public static const const_1733:int = 2;
      
      private static var var_978:Array = new Array();
      
      private static var var_791:Array = new Array();
      ...
      protected var var_269:URLRequest;
      
      protected var _manifest:XML;
      
      protected var var_713:Class;
      
      protected var var_510:uint = 0;
      ...
      protected var var_3127:uint = 0;
      
      protected var var_3054:Boolean = true;
      ...
      protected static function queue(param1:LibraryLoader) : void
      {
         ...
         {
            param1.var_44.load(param1.var_269,param1._context);
         }
      }
      ...
      protected static function throttle() : void
      {
         ...
         if(const_1180)
         {
            ...
            while(_loc2_ > -1)
            {
               _loc1_ = var_791[_loc2_] as LibraryLoader;
               if(_loc1_ && (_loc1_.ready || _loc1_.disposed))
               {
                  var_791.splice(_loc2_,1);
               }
               _loc2_--;
            }
            while(var_791.length < const_1733 && false)
            {
               _loc1_ = var_978.shift() as LibraryLoader;
               if(!_loc1_.ready && !_loc1_.disposed)
               {
                  var_791.push(_loc1_);
                  _loc1_.var_44.load(_loc1_.var_269,_loc1_._context);
               }
            }
         }
      }
      ...
      {
         return !!this.var_269 ? this.var_269.url : null;
      }
      ...
      {
         return !!this.var_1148 ? uint(this.var_1419 - this.var_2263) : uint(getTimer() - this.var_2263);
      }
      ...
      override public function dispose() : void
      {
         if(!disposed)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DISPOSE,this.var_430,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
            try
            {
               this.var_44.close();
            }
      ...
      {
         this.var_269 = param1;
         this._name = parseNameFromUrl(this.var_269.url);
         this.var_1148 = false;
      ...
      {
         if(this.var_1147 && !_disposed)
         {
            this.var_1147 = false;
            if(!this.var_1148 && this.var_269)
            {
               this.var_1419 = -1;
               this.var_2263 = getTimer();
               queue(this);
            }
         }
      }
      ...
      {
         if(!this.var_1148 && !_disposed && this.var_1418 > 0)
         {
            try
            {
               this.var_44.close();
            ...
            this.addRequestCounterToUrlRequest(this.var_269,const_1892 - this.var_1418);
            if(this.var_3054)
            {
               this.var_269.requestHeaders.push(new URLRequestHeader("pragma","no-cache"));
               this.var_269.requestHeaders.push(new URLRequestHeader("Cache-Control","no-cache"));
               this.var_3054 = false;
            }
            --this.var_1418;
            this.var_510 = 0;
            this.var_44.load(this.var_269,this._context);
            return true;
         }
         return false;
      }
      ...
      protected function loadEventHandler(param1:Event) : void
      {
         ...
         switch(param1.type)
         {
            ...
            case HTTPStatusEvent.HTTP_STATUS:
               ...
               this.debug("Load event STATUS " + this.var_430 + " for file \"" + this.url + "\"");
               break;
            case Event.UNLOAD:
               this.debug("Load event UNLOAD for file \"" + this.url + "\"");
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_UNLOAD,this.var_430,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case ProgressEvent.PROGRESS:
               this.debug("Load event PROGRESS for file \"" + this.url + "\"  bytes: " + this.bytesLoaded + "/" + this.bytesTotal);
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_PROGRESS,this.var_430,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case IOErrorEvent.IO_ERROR:
               this.debug("Load event IO ERROR for file \"" + this.url + "\"");
               if(!this.handleHttpStatus(this.var_430))
               {
                  this.var_1419 = getTimer();
            ...
            case TimerEvent.TIMER:
               ...
               if(!_disposed)
               {
                  this.var_510 = LibraryLoader.name_5;
         ...
         if(this.var_510 == LibraryLoader.const_2042)
         {
         ...
         if(this.var_510 == LibraryLoader.name_5)
         {
            if(this.prepareLibrary())
            {
               ...
               this.removeEventListeners();
               throttle();
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE,this.var_430,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
      ...
      protected function prepareLibrary() : Boolean
      {
         ...
         this.var_713 = this.getDefinition(this._name) as Class;
         if(this.var_713 == null)
         {
            if(!this.retry())
            {
               this.failure("Failed to find resource class \"" + this._name + "\" from library " + this.var_269.url + "!");
            }
            return false;
         }
         ...
         try
         {
            xmlClass = this.var_713.manifest as Class;
            if(xmlClass == null)
            {
               return false;
            }
      ...
      {
         Core.debug(param1);
         this.var_3052 = param1;
         if(this._debug)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DEBUG,this.var_430,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         }
      }
      ...
      protected function failure(param1:String) : void
      {
         ...
         throttle();
         dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR,this.var_430,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         dispatchEvent(new Event(LIBRARY_LOADER_FINALIZE));
      }
