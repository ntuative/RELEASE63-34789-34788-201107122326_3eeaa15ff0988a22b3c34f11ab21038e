package com.sulake.core.window.components
{
   ...
   public class ActivatorController extends ContainerController
   {
      ...
      {
         if(param2.type == WindowEvent.const_346)
         {
            this.setActiveChild(param1 as IWindow);
         }
         else if(param2.type == WindowEvent.const_1205)
         {
            return true;
         }
      ...
      public function setActiveChild(param1:IWindow) : IWindow
      {
         ...
         var _loc2_:IWindow = this.var_934;
         if(this.var_934 != param1)
         {
            if(this.var_934 != null)
            {
               if(!this.var_934.disposed)
               {
                  this.var_934.deactivate();
               }
            }
            this.var_934 = param1;
