package com.sulake.core.runtime
{
   ...
   public final class CoreComponentContext extends ComponentContext implements ICore
   {
      ...
      private static const const_1636:String = "asset-libraries";
      
      private static const const_1633:String = "library";
      
      private static const const_1637:String = "service-libraries";
      
      private static const const_1635:String = "library";
      
      private static const const_1630:String = "component-libraries";
      
      private static const const_1634:String = "library";
      
      private static const const_1632:String = "error_data";
      
      private static const const_1631:String = "error_cat";
      
      private static const const_1638:String = "error_desc";
       
      
      private var var_407:LibraryLoaderQueue;
      
      private var var_875:IEventDispatcher;
      
      private var var_583:uint;
      
      private var var_408:Function;
      
      private var var_232:Profiler;
      ...
      public function CoreComponentContext(param1:DisplayObjectContainer, param2:uint)
      {
         ...
         this.var_305 = new Array();
         this.var_723 = new Array();
         var_429 = param1;
         var _loc3_:int = 0;
         while(_loc3_ < CoreComponentContext.const_107)
         {
            this.var_305.push(new Array());
            this.var_723.push(0);
            _loc3_++;
         }
         this._lastUpdateTimeMs = getTimer();
         attachComponent(this,[new IIDCore()]);
         var_429.addEventListener(Event.ENTER_FRAME,this.onEnterFrame);
         switch(param2 & 0)
         {
            case Core.const_1269:
               debug("Core; using simple frame update handler");
               this.var_408 = this.simpleFrameUpdateHandler;
               break;
            case Core.const_919:
               debug("Core; using complex frame update handler");
               this.var_408 = this.complexFrameUpdateHandler;
               break;
            case Core.const_930:
               debug("Core; using profiler frame update handler");
               this.var_408 = this.profilerFrameUpdateHandler;
               this.var_232 = new Profiler(this);
               attachComponent(this.var_232,[new IIDProfiler()]);
               var_429.addChild(new ProfilerViewer(this.var_232));
               break;
            case Core.const_516:
               debug("Core; using experimental frame update handler");
               this.var_408 = this.experimentalFrameUpdateHandler;
               break;
            case Core.CORE_SETUP_DEBUG:
               debug("Core; using debug frame update handler");
               this.var_408 = this.debugFrameUpdateHandler;
         }
      }
      
      public function getNumberOfFilesPending() : uint
      {
         return this.var_407.length;
      }
      
      public function getNumberOfFilesLoaded() : uint
      {
         return this.var_583 - this.getNumberOfFilesPending();
      }
      ...
      override public function dispose() : void
      {
         ...
         if(!disposed)
         {
            ...
            try
            {
               ...
               {
                  receivers = this.var_305[i] as Array;
            ...
            if(var_429)
            {
               var_429.removeEventListener(Event.ENTER_FRAME,this.onEnterFrame);
               var_429 = null;
            }
      ...
      override public function error(param1:String, param2:Boolean, param3:int = -1, param4:Error = null) : void
      {
         if(param4)
         {
            ErrorReportStorage.setParameter(const_1632,String(param4.getStackTrace()));
         }
         ErrorReportStorage.setParameter(const_1631,String(param3));
         ErrorReportStorage.setParameter(const_1638,param1);
      ...
      public function readConfigDocument(param1:XML, param2:IEventDispatcher = null) : void
      {
         ...
         this.var_875 = loadingEventDelegate;
         if(this.var_407 == null)
         {
            this.var_407 = new LibraryLoaderQueue(_debug);
         }
         try
         {
            node = config.child(const_1636)[0];
            if(node != null)
            {
               list = node.child(const_1633);
         ...
         {
            error("Failed to parse asset libraries from config xml!",true,Core.const_219,e);
         }
         try
         {
            node = config.child(const_1637)[0];
            if(node != null)
            {
               list = node.child(const_1635);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         {
            error("Failed to parse interfaces from config xml!",true,Core.const_219,e);
         }
         try
         {
            node = config.child(const_1630)[0];
            if(node != null)
            {
               list = node.child(const_1634);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         {
            error("Failed to parse components from config xml!",true,Core.const_219,e);
         }
         if(!disposed)
         {
      ...
      private function errorInLoadingProcess(param1:LibraryLoaderEvent = null) : void
      {
         ...
         this.error("Failed to download library \"" + _loc2_.url + "\" HTTP status " + param1.status + ": " + _loc2_.getLastErrorMessage(),true,Core.const_1322);
         if(!disposed)
         {
      ...
      private function finalizeLoadingEventDelegate() : void
      {
         if(this.var_875)
         {
            this.var_875.dispatchEvent(new Event(Event.COMPLETE));
         }
      }
      ...
      private function updateLoadingProgress(param1:LibraryLoaderEvent = null) : void
      {
         ...
         if(this.var_875)
         {
            ...
            this.var_875.dispatchEvent(new LibraryProgressEvent(_loc2_.url,param1.bytesLoaded,param1.bytesTotal,_loc2_.elapsedTime));
         }
      }
         ...
         if(param1 != null)
         {
            if(param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE || param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR)
            {
               ...
               if(!disposed)
               {
                  if(this.var_875)
                  {
                     this.var_875.dispatchEvent(new LibraryProgressEvent(_loc2_.url,this.var_583 - this.var_407.length,this.var_583,_loc2_.elapsedTime));
                  }
               }
            }
         }
      ...
      override public function registerUpdateReceiver(param1:IUpdateReceiver, param2:uint) : void
      {
         ...
         var _loc3_:int = !!this.var_232 ? 0 : this.var_1634 & 0;
         if(_loc3_ == Core.const_516)
         {
            this.var_305[param2].push(new UpdateDelegate(param1,this,param2));
         }
         else
         {
            this.var_305[param2].push(param1);
         }
      }
      ...
      override public function removeUpdateReceiver(param1:IUpdateReceiver) : void
      {
         ...
         var _loc4_:int = !!this.var_232 ? 0 : this.var_1634 & 0;
         var _loc5_:int = 0;
         while(_loc5_ < CoreComponentContext.const_107)
         {
            _loc3_ = this.var_305[_loc5_] as Array;
            if(_loc4_ == Core.const_516)
            {
      ...
      private function onEnterFrame(param1:Event) : void
      {
         ...
         this.var_408(_loc2_,_loc2_ - this._lastUpdateTimeMs);
      ...
      private function simpleFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         {
            this.var_723[priority] = 0;
            receivers = this.var_305[priority];
            index = 0;
            length = receivers.length;
            while(index != length)
            {
      ...
      private function complexFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         var maxTimePerFrame:uint = 1000 / var_429.stage.frameRate;
         priority = 0;
         while(priority < CoreComponentContext.const_107)
         {
            t = getTimer();
            ...
            {
               if(this.var_723[priority] < priority)
               {
                  ++this.var_723[priority];
                  skip = true;
               }
            }
            if(!skip)
         ...
         while(priority < CoreComponentContext.const_107)
         {
            ...
            while(index != length)
            {
               ...
               else
               {
                  try
                  {
                     this.var_232.update(receiver,msSinceLastUpdate);
                  }
                  catch(e:Error)
                  {
                     error("Error in update receiver \"" + getQualifiedClassName(receiver) + "\": " + e.message,true,e.errorID,e);
      ...
      public function setProfilerMode(param1:Boolean) : void
      {
         ...
         else
         {
            if(this.var_232)
            {
               detachComponent(this.var_232);
               this.var_232.dispose();
               this.var_232 = null;
            }
            switch(this.var_1634 & 0)
            {
               case Core.const_1269:
                  this.var_408 = this.simpleFrameUpdateHandler;
                  break;
